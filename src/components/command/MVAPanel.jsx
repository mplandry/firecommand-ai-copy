...
import { Car, Plus, Trash2, User, Ambulance, Camera, X, ChevronDown, ChevronUp, Users, Mic, MicOff, Loader2, ExternalLink, Search, Phone, MapPin, ScanLine, Share2, FileText } from 'lucide-react';
...
// ── Report generation ─────────────────────────────────────────────────────────
function generateReportText(data, incidentId) {
  const now = new Date();
  ...
  const lines = [
    ...
    `Vehicles: ${data.vehicles.length}   Patients: ${data.patients.length}`,
    '',
  ].filter(l => l !== null);
  ...
  data.vehicles.forEach((v, vIdx) => {
    ...
    lines.push(`───────────────────────────────────────`);
    lines.push(`VEHICLE ${vIdx + 1}`);
    lines.push(`───────────────────────────────────────`);
    if (v.plate || v.state) lines.push(`Plate:   ${[v.state, v.plate].filter(Boolean).join(' ')}`);
    if (v.year || v.make || v.model) lines.push(`Vehicle: ${[v.year, v.make, v.model].filter(Boolean).join(' ')}`);
    if (v.color)  lines.push(`Color:   ${v.color}`);
    if (v.vin)    lines.push(`VIN:     ${v.vin}`);
    if (v.insuranceCompany) {
      lines.push('');
      lines.push('Insurance:');
      lines.push(`  Company: ${v.insuranceCompany}${v.insuranceCity ? ` (${v.insuranceCity})` : ''}`);
      if (v.insurancePhone)   lines.push(`  Phone:   ${v.insurancePhone}`);
      if (v.insuranceAddress) lines.push(`  Address: ${v.insuranceAddress}`);
    }
    if (vPatients.length > 0) {
      lines.push('');
      vPatients.forEach((p, pIdx) => {
        lines.push(`  Patient ${pIdx + 1}${p.position ? ` — ${p.position}` : ''}`);
        if (p.name)     lines.push(`    Name:     ${p.name}`);
        if (p.sex)      lines.push(`    Sex:      ${p.sex.charAt(0).toUpperCase() + p.sex.slice(1)}`);
        if (p.dob)      lines.push(`    DOB:      ${p.dob} (Age ${Math.floor((Date.now() - new Date(p.dob)) / 31557600000)})`);
        if (p.severity) lines.push(`    Severity: ${p.severity.toUpperCase()}`);
        if (p.hospital) lines.push(`    Hospital: ${p.hospital}`);
  ...
  const unassigned = data.patients.filter(p => !p.vehicleId);
  if (unassigned.length > 0) {
    lines.push(`───────────────────────────────────────`);
    lines.push('ADDITIONAL PATIENTS');
    lines.push(`───────────────────────────────────────`);
    unassigned.forEach((p, idx) => {
      lines.push(`Patient ${idx + 1}`);
      if (p.name)     lines.push(`  Name:     ${p.name}`);
      if (p.sex)      lines.push(`  Sex:      ${p.sex.charAt(0).toUpperCase() + p.sex.slice(1)}`);
      if (p.dob)      lines.push(`  DOB:      ${p.dob} (Age ${Math.floor((Date.now() - new Date(p.dob)) / 31557600000)})`);
      if (p.severity) lines.push(`  Severity: ${p.severity.toUpperCase()}`);
  ...
  lines.push('═══════════════════════════════════════');
  lines.push('Generated by Fire Command AI');
  return lines.join('\n');
}

function openReportPDF(data, incidentId) {
  const now = new Date();
  const dateStr = now.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
  const timeStr = now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });

  const vehicleBlocks = data.vehicles.map((v, vIdx) => {
    const vPatients = data.patients.filter(p => p.vehicleId === v.id);
    const patientRows = vPatients.map((p, pIdx) => `
      <div class="patient">
        <div class="patient-header">Patient ${pIdx + 1}${p.position ? ` &mdash; ${p.position}` : ''} &nbsp;
          <span class="sev sev-${p.severity}">${(p.severity || 'minor').toUpperCase()}</span>
        </div>
        <table class="fields">
          ${p.name     ? `<tr><td>Name</td><td>${p.name}</td></tr>` : ''}
          ${p.sex      ? `<tr><td>Sex</td><td>${p.sex.charAt(0).toUpperCase() + p.sex.slice(1)}</td></tr>` : ''}
          ${p.dob      ? `<tr><td>DOB</td><td>${p.dob} (Age ${Math.floor((Date.now() - new Date(p.dob)) / 31557600000)})</td></tr>` : ''}
          ${p.hospital ? `<tr><td>Hospital</td><td>${p.hospital}</td></tr>` : ''}
    ...
    return `
      <div class="vehicle-block">
        <div class="vehicle-header">Vehicle ${vIdx + 1}</div>
        <table class="fields">
          ${(v.plate || v.state) ? `<tr><td>Plate</td><td>${[v.state, v.plate].filter(Boolean).join(' ')}</td></tr>` : ''}
          ${(v.year || v.make || v.model) ? `<tr><td>Vehicle</td><td>${[v.year, v.make, v.model].filter(Boolean).join(' ')} ${v.color ? `· ${v.color}` : ''}</td></tr>` : ''}
          ${v.vin ? `<tr><td>VIN</td><td>${v.vin}</td></tr>` : ''}
          ${v.insuranceCompany ? `<tr><td>Insurance</td><td>${v.insuranceCompany}${v.insuranceCity ? ` (${v.insuranceCity})` : ''}</td></tr>` : ''}
          ${v.insurancePhone ? `<tr><td>Ins. Phone</td><td>${v.insurancePhone}</td></tr>` : ''}
          ${v.insuranceAddress ? `<tr><td>Ins. Address</td><td>${v.insuranceAddress}</td></tr>` : ''}
        </table>
        ${patientRows}
      </div>`;
  }).join('');
  ...
  const unassignedBlock = unassigned.length === 0 ? '' : `
    <div class="vehicle-block">
      <div class="vehicle-header">Additional Patients</div>
      ${unassigned.map((p, idx) => `
        <div class="patient">
          <div class="patient-header">Patient ${idx + 1} &nbsp;
            <span class="sev sev-${p.severity}">${(p.severity || 'minor').toUpperCase()}</span>
          </div>
          <table class="fields">
            ${p.name     ? `<tr><td>Name</td><td>${p.name}</td></tr>` : ''}
...
<title>MVA Incident Report${incidentId ? ` — ${incidentId}` : ''}</title>
<style>
  body { font-family: Arial, sans-serif; font-size: 12px; color: #111; margin: 0; padding: 24px; }
  h1 { font-size: 18px; margin: 0 0 4px; }
  .meta { color: #555; font-size: 11px; margin-bottom: 16px; }
  .vehicle-block { border: 1px solid #ccc; border-radius: 6px; margin-bottom: 16px; overflow: hidden; page-break-inside: avoid; }
  .vehicle-header { background: #1a2233; color: #fff; font-weight: bold; padding: 6px 12px; font-size: 13px; }
  table.fields { width: 100%; border-collapse: collapse; padding: 8px 12px; display: table; }
  table.fields td { padding: 3px 8px; vertical-align: top; }
  table.fields td:first-child { color: #555; width: 100px; font-size: 10px; text-transform: uppercase; letter-spacing: .04em; }
  .patient { border-top: 1px solid #e5e7eb; margin: 0; }
  .patient-header { background: #f3f4f6; padding: 4px 12px; font-weight: 600; font-size: 11px; }
  .sev { display: inline-block; padding: 1px 6px; border-radius: 4px; font-size: 10px; font-weight: bold; }
  .sev-minor    { background: #d1fae5; color: #065f46; }
  .sev-moderate { background: #fef3c7; color: #92400e; }
  .sev-critical { background: #fee2e2; color: #991b1b; }
  .sev-deceased { background: #f3f4f6; color: #374151; }
  .footer { margin-top: 24px; font-size: 10px; color: #999; border-top: 1px solid #e5e7eb; padding-top: 8px; }
  @media print { body { padding: 0; } }
</style>
</head><body>
<h1>MVA Incident Report</h1>
<div class="meta">${dateStr} &nbsp;·&nbsp; ${timeStr}${incidentId ? ` &nbsp;·&nbsp; Incident ${incidentId}` : ''} &nbsp;·&nbsp; ${data.vehicles.length} vehicle(s) &nbsp;·&nbsp; ${data.patients.length} patient(s)</div>
${vehicleBlocks}
${unassignedBlock}
<div class="footer">Generated by Fire Command AI</div>
    ...
    <div className="space-y-6 p-4 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {(data.vehicles.length > 0 || data.patients.length > 0) && (
            <>
              {typeof navigator.share === 'function' && (
                <Button size="sm" variant="outline" className="h-7 text-xs gap-1.5"
                  onClick={() => navigator.share({
                    title: `MVA Incident Report${incidentId ? ` — ${incidentId}` : ''}`,
                    text: generateReportText(data, incidentId),
                  }).catch(() => {})}>
                  <Share2 className="w-3 h-3" /> Share
                </Button>
              )}
              <Button size="sm" variant="outline" className="h-7 text-xs gap-1.5"
                onClick={() => openReportPDF(data, incidentId)}>
                <FileText className="w-3 h-3" /> PDF / Print
              </Button>
            </>
          )}
        </div>
